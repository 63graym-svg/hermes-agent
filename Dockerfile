...
#
# `uv sync --frozen --no-install-project --extra all --extra honcho --extra messaging`
...
RUN touch ./README.md
RUN uv sync --frozen --no-install-project --extra all --extra honcho --extra messaging --extra anthropic --extra bedrock --extra azure-identity --extra hindsight --extra matrix
...
# VOLUME [ "/opt/data" ]  # Railway 不支援 Dockerfile VOLUME 指令；改用已掛好的 Railway Volume（/opt/data，效果相同）
